function [modout]= EMfit_ms_par(rootfile, modelID, bounds,attempt,max_attempts)
...
% OUTPUT:      - fitted model
%
% DEPENDENCIES: - get_npar
%               - mod_ms_all
%
% SL Feb 2025: added (max) number of attempts for the try-catch and changed
% prior.logpdf based on TV
...
for is = 1:n_subj
   ...
   fit.ntrials(is) = sum((rootfile.beh{is}.choice~=2)); % exclude missed trials
end
...
posterior.mu        = abs(.1.*randn(fit.npar,1));%JC randn(fit.npar,1);%TV % start with positive values; define posterior because it is assigned to prior @ beginning of loop
...
%    sigma = sqrt(prior.sigma);                                                 % SL 17-02-25: %for some reason, the output from compGauss_ms.m returns (prior.sigma) is actually the variance (sigma^2), not the standard deviation (sigma). square-rooted it here to make the formula below clearer.
%    prior.logpdf = @(x) sum((-log(sigma)) - (0.5.*log(2*pi)) - (((x-prior.mu).^2) ./ (2.*(sigma.^2)))); 
   ...
   for is = 1:n_subj                                                          
      ...
      while ex<0                                                              % just to make sure the fitting is done
           q = .1*randn(fit.npar,1); %+ .01;                                   % free parameters set to random, SL 17-02-2025 added + .01     
%           if iiter == 1                                                       % SL 17-02-25: using the priors for starting parameter values (except on the first iteration)
%               q = randn(fit.npar, 1);
%           else
%               q = normrnd(prior.mu, sqrt(prior.sigma), fit.npar, 1);
%           end
